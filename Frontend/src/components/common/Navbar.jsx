...
function Navbar() {
  ...
  const pharmacistLinks = (userId) => [
    { text: "لوحة التحكم", path: "/pharmacist" },
    { text: "إدارة المخزون", path: `/pharmacist/${userId}/inventory` },
    { text: "إدارة الاعلانات", path: `/pharmacist/${userId}/AdsManager` },
  ];

  const getNavLinks = () => {
    if (!user) return citizenLinks;

    switch (user.role?.toLowerCase()) {
      case "syndicate":
        return syndicateLinks;
      case "pharmacist":
        return pharmacistLinks(user?.pharmacy_id || user?.pharmacyId || "");
      default:
        return citizenLinks;
    }
  };
          ...
          <div className="flex justify-between items-center h-16 md:h-20 w-full">
            ...
            <div className="flex items-center gap-3">
              ...
              <Link to="/" className="flex items-center gap-2 group">
                ...
                <span className="text-xl md:text-2xl font-black text-content-main">دوائي</span>
              </Link>
            </div>
            ...
            {user && (
              <div className="flex flex-col leading-tight md:hidden">
                <span className="text-xs font-bold text-content-light">مرحبا {user.role}</span>
                <button
                  onClick={logout}
      ...
      >
        <div className="p-6 flex flex-col h-full">
          ...
          <div className="flex justify-between items-center mb-10">
            <span className="text-xl font-black text-content-main">القائمة</span>
            <button
              onClick={() => setMobileOpen(false)}
